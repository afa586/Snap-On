...
###Define
$Account = "snaponglobal\beservice"
$SearchAccount = $Account -replace "snaponglobal\\"
...
To list all Tasks which run as the account:
...
$Results = @()
    ...
    {
    $Results += Invoke-Command -ComputerName $Server -ScriptBlock {Get-ScheduledTask | Where-Object { $_.Principal.UserId -like "*$Using:SearchAccount*" -and $_.TaskName -notlike "Optimize Start*" -and $_.TaskName -notlike "*User_Feed*"}} |select PsComputerName,TaskName,State,@{n = 'UserID';e = {$_.Principal.UserId}}
    }
$TextBox1.Text = $Results|Out-String
$Results |Export-Csv -Path $ListReport -NoTypeInformation
...
if (!$Password)
    ...
    foreach ($Server in $Servers)
        {
        $Tasks = Invoke-Command -ComputerName $Server -ScriptBlock {Get-ScheduledTask | Where-Object { $_.Principal.UserId -like "*$Using:SearchAccount*" -and $_.TaskName -notlike "Optimize Start*" -and $_.TaskName -notlike "*User_Feed*"}} |select TaskName,State,PsComputerName,TaskPath
            foreach ($Task in $Tasks)
            {
             $Block = {Set-ScheduledTask -TaskPath $Using:Task.TaskPath -TaskName $Using:Task.Taskname -User $Using:Account -Password $Using:Password |Out-Null}
                try
                {
             ...
             $Results += $Task | select TaskName,PsComputername,@{n="Result";e={$Status}}   
            }
         }
    $TextBox1.Text = $Results |Out-String
